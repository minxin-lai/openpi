...
run_dir="runs/${run_tag}/${step_label}_${ts}"
...
mkdir -p "$(dirname "${launcher_log}")" "$(dirname "${server_log}")" "$(dirname "${client_log}")" "${video_out}"

echo "=== OpenPI Run (server + client) ==="
echo "ckpt: ${ckpt_dir}"
...
if [[ -n "${opt_config}" ]]; then
  server_args+=(--opt-config "${opt_config}")
fi
