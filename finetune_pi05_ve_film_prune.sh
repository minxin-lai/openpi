...
echo ""
echo "vla-opt: ve_film_num_blocks=${ve_film_num_blocks} ste_prune_k=${ste_prune_k} stage=${ste_prune_stage}"
echo ""
...
CUDA_VISIBLE_DEVICES="${gpus}" "${python_bin}" -m torch.distributed.run --standalone --nproc_per_node="${nproc_per_node}" \
  ...
  --ste-prune --ste-prune-k "${ste_prune_k}" --ste-prune-stage "${ste_prune_stage}" --ste-prune-switch-step "${ste_prune_switch_step}" \
