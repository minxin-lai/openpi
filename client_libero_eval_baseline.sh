...
# LIBERO eval client (baseline defaults)
#
# 你只需要记住：
#   1) 先起 server：bash server_pi05_libero_baseline.sh
#   2) 再跑 client：bash client_libero_eval_baseline.sh
#
# 输出：
# - 视频：runs/libero/videos/baseline/...
# - 日志：runs/libero/logs/baseline/...
...
usage() {
  cat <<'EOF'
Usage:
  bash client_libero_eval_baseline.sh [options...]
...
Options:
  ...
  --video-out <path>  default: runs/libero/videos/baseline/<suite>_<ts>
  --log <path>        default: runs/libero/logs/baseline/<suite>_<ts>.log
EOF
}
...
if [[ -z "${video_out}" ]]; then
  video_out="runs/libero/videos/baseline/${suite}_${ts}"
fi
if [[ -z "${log_path}" ]]; then
  log_path="runs/libero/logs/baseline/${suite}_${ts}.log"
fi
...
echo "=== OpenPI LIBERO Client (baseline) ==="
echo "host: ${host}"
echo "port: ${port}"
echo "suite: ${suite}"
echo "trials: ${trials}"
